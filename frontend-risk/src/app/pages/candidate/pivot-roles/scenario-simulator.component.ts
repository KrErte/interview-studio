    ...
    <div class="border border-stone-200 bg-white overflow-hidden">
      ...
      <div class="px-5 pt-5 pb-4 border-b border-stone-200">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-[10px] font-bold text-stone-400 uppercase tracking-widest">What-If Simulator</h3>
            <p class="text-xs text-stone-500">See how different choices affect your career trajectory</p>
          </div>
        </div>
      </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          <button
            *ngFor="let scenario of scenarios"
            type="button"
            class="group relative p-4 border text-left transition-all duration-300"
          ...
          >
            ...
            <div
              *ngIf="selectedScenario?.id === scenario.id"
              class="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-red-600 flex items-center justify-center"
            >
            ...
            <div class="flex items-start justify-between gap-3 mb-3">
              <span class="text-sm font-semibold text-stone-900">{{ scenario.action }}</span>
              <span class="px-2 py-0.5 text-[10px] font-bold bg-stone-100 text-stone-400 uppercase tracking-widest">
                {{ scenario.timeInvestment }}
              </span>
            </div>

            <p class="text-xs text-stone-500 mb-3">{{ scenario.description }}</p>

            <!-- Quick impact preview -->
            <div class="flex items-center gap-4 text-xs">
              <div class="flex items-center gap-1">
                <span class="text-stone-400">Risk:</span>
                <span [ngClass]="scenario.riskChange < 0 ? 'text-stone-900' : 'text-red-600'">
                  {{ scenario.riskChange > 0 ? '+' : '' }}{{ scenario.riskChange }}%
                </span>
              </div>
              <div class="flex items-center gap-1">
                <span class="text-stone-400">Salary:</span>
                <span [ngClass]="scenario.salaryChange > 0 ? 'text-stone-900' : 'text-red-600'">
      ...
      <div *ngIf="simulationResult" class="px-5 pb-5">
        <div class="p-5 bg-stone-50 border border-stone-200">
          <div class="flex items-center justify-between mb-4">
            <h4 class="text-sm font-bold text-stone-900">Simulation Result</h4>
            <span class="text-xs text-stone-400">{{ simulationResult.confidence }}% confidence</span>
          </div>

          <!-- Before/After comparison -->
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="p-3 bg-white border border-stone-200">
              <div class="text-xs text-stone-400 mb-1">Current State</div>
              <div class="flex items-baseline gap-2">
                <span class="text-2xl font-mono font-bold text-stone-400">{{ simulationResult.originalRisk }}%</span>
                <span class="text-xs text-stone-400">risk</span>
              </div>
              <div class="text-sm text-stone-500">\${{ simulationResult.originalSalary | number }}k/yr</div>
            </div>
            <div class="p-3 bg-white border border-stone-200">
              <div class="text-xs text-stone-900 mb-1">After {{ selectedScenario?.timeInvestment }}</div>
              <div class="flex items-baseline gap-2">
                <span class="text-2xl font-mono font-bold" [ngClass]="simulationResult.newRisk < simulationResult.originalRisk ? 'text-stone-900' : 'text-red-600'">
                  {{ simulationResult.newRisk }}%
                </span>
                <span class="text-xs text-stone-400">risk</span>
              </div>
              <div class="text-sm text-stone-900">\${{ simulationResult.projectedSalary | number }}k/yr</div>
            </div>
          </div>

          <!-- Visual comparison bar -->
          <div class="relative h-8 bg-stone-100 overflow-hidden mb-4">
            <div
              class="absolute inset-y-0 left-0 bg-stone-300 transition-all duration-500"
              [style.width.%]="simulationResult.originalRisk"
            ></div>
            <div
              class="absolute inset-y-0 left-0 transition-all duration-500"
              [ngClass]="simulationResult.newRisk < simulationResult.originalRisk ? 'bg-stone-900' : 'bg-red-600'"
          ...
          <!-- Tradeoffs -->
          <div class="space-y-2">
            <div class="text-[10px] text-stone-400 uppercase tracking-widest font-bold">Tradeoffs to Consider</div>
            <div *ngFor="let tradeoff of simulationResult.tradeoffs" class="flex items-start gap-2 text-sm text-stone-600">
          ...
          <button
            type="button"
            class="mt-4 w-full py-3 bg-red-600 text-sm font-bold text-white hover:bg-red-700 transition-colors"
      ...
      <!-- Compare mode hint -->
      <div *ngIf="!simulationResult" class="px-5 pb-5">
        <div class="p-4 border border-dashed border-stone-300 text-center">
          <p class="text-sm text-stone-500">Select a scenario above to simulate its impact on your career</p>
        </div>
      </div>
    </div>
  `
...
export class ScenarioSimulatorComponent implements OnInit {
  ...
  getScenarioClasses(scenario: Scenario): string {
    ...
    if (isSelected) {
      return 'border-red-600 bg-red-50 ring-1 ring-red-200';
    }
    if (scenario.riskChange < 0) {
      return 'border-stone-200 bg-white hover:border-stone-900 hover:bg-stone-50';
    }
