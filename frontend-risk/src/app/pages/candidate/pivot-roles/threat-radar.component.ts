...
@Component({
  ...
  template: `
    <div class="relative border border-stone-200 bg-white shadow-sm overflow-hidden">
      <!-- Header -->
      <div class="relative z-10 px-5 pt-5 pb-3 border-b border-stone-200">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-sm font-black text-stone-900 uppercase tracking-widest">Career Threat Radar</h3>
              <p class="text-xs text-stone-500">Real-time vulnerability scan</p>
            </div>
          </div>
          <div class="text-right">
            <div class="text-2xl font-mono font-bold" [ngClass]="threatLevelColor">{{ overallThreat }}%</div>
            <div class="text-[10px] uppercase tracking-widest text-stone-400 font-bold">threat level</div>
          </div>
        </div>
      </div>
      ...
      <div class="relative z-10 p-5">
        <div class="grid gap-2">
          ...
          >
            <!-- Threat row -->
            <div
              class="relative flex items-center gap-3 p-3 border transition-all duration-300 cursor-pointer"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <span class="text-sm font-bold text-stone-900">{{ threat.label }}</span>
                  <span
                ...
                <p class="text-xs text-stone-500 mt-0.5 truncate">{{ threat.description }}</p>
              </div>

              <!-- ETA -->
              <div class="flex-shrink-0 text-right">
                <div class="text-xs font-mono text-stone-600">{{ threat.eta }}</div>
                <div class="text-[10px] text-stone-400 uppercase">until impact</div>
              </div>
            ...
            >
              <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span class="text-stone-500">Automation Probability</span>
                  <div class="mt-1 h-2 bg-stone-200 rounded-full overflow-hidden">
                    <div
                ...
                <div>
                  <span class="text-stone-500">Market Readiness</span>
                  <div class="mt-1 h-2 bg-stone-200 rounded-full overflow-hidden">
                    <div
                      class="h-full bg-stone-900 rounded-full transition-all duration-700"
      ...
      <!-- Footer scan status -->
      <div class="relative z-10 px-5 py-3 border-t border-stone-200 flex items-center justify-between">
...
export class ThreatRadarComponent implements OnInit, OnDestroy {
  ...
  get statusColor(): string {
    ...
    if (threat >= 70) return 'bg-red-600';
  ...
  get threatLevelColor(): string {
    const threat = this.overallThreat;
    if (threat >= 70) return 'text-red-600';
    if (threat >= 50) return 'text-amber-700';
  ...
  getThreatTextColor(threat: ThreatVector): string {
    if (threat.severity >= 70) return 'text-red-600';
    if (threat.severity >= 50) return 'text-amber-700';
    if (threat.severity >= 30) return 'text-stone-600';
    return 'text-stone-900';
  }

  getThreatBarColor(threat: ThreatVector): string {
    if (threat.severity >= 70) return 'bg-red-600';
    if (threat.severity >= 50) return 'bg-amber-500';
    if (threat.severity >= 30) return 'bg-stone-600';
    return 'bg-stone-900';
  }
