...
@Component({
  ...
  template: `
    <div class="max-w-3xl mx-auto px-4 py-8">
      <h1 class="text-3xl font-bold text-white mb-2">AI Career Mentor</h1>
      <p class="text-slate-400 mb-8">Get personalized career advice powered by AI. Ask questions, get actionable guidance.</p>
      ...
      @if (!sessionStarted()) {
        <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-300 mb-1">Main Challenge</label>
          ...
          <button (click)="startMentoring()" [disabled]="loading() || !targetRole"
            class="w-full py-3 rounded-xl font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg hover:shadow-purple-500/40 transition-all disabled:opacity-50">
      ...
      @if (sessionStarted()) {
        <div class="space-y-6">
          <!-- Career Outlook -->
          @if (careerOutlook()) {
            <div class="rounded-xl border border-purple-900/50 bg-purple-950/20 p-5">
              <h3 class="font-semibold text-purple-400 mb-2">Career Outlook</h3>
              <p class="text-sm text-slate-300">{{ careerOutlook() }}</p>
            </div>
          }
          ...
          @if (actionItems().length) {
            <div class="rounded-xl border border-emerald-900/50 bg-emerald-950/20 p-5">
              <h3 class="font-semibold text-emerald-400 mb-3">Action Items</h3>
              <ul class="space-y-2">
          ...
          @if (resourceLinks().length) {
            <div class="rounded-xl border border-slate-800 bg-slate-900/80 p-5">
              <h3 class="font-semibold text-white mb-3">Recommended Resources</h3>
              <ul class="space-y-2">
                @for (link of resourceLinks(); track link) {
                  <li class="text-sm text-slate-300 flex items-start gap-2">
                    <span class="text-cyan-400 mt-0.5">&#x2192;</span> {{ link }}
                  </li>
                }
              </ul>
            </div>
          ...
          @for (msg of chatMessages(); track $index) {
            <div [class]="msg.role === 'mentor'
              ...
              : 'bg-purple-950/30 border border-purple-900/50 rounded-2xl p-5 ml-8'">
              <div class="text-xs font-semibold mb-2" [class]="msg.role === 'mentor' ? 'text-purple-400' : 'text-emerald-400'">
          ...
          <div class="space-y-3">
            <textarea [(ngModel)]="followUpMessage" rows="3" placeholder="Ask your career mentor a question..."
              class="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 resize-none"></textarea>
            <div class="flex gap-3">
              <button (click)="sendMessage()" [disabled]="loading() || !followUpMessage"
                class="flex-1 py-3 rounded-xl font-bold bg-purple-500 text-white hover:bg-purple-400 transition-all disabled:opacity-50">
