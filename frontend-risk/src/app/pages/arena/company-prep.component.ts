...
@Component({
  ...
  template: `
    <div class="max-w-3xl mx-auto px-4 py-8">
      <h1 class="text-3xl font-bold text-white mb-2">Company-Specific Interview Prep</h1>
      <p class="text-slate-400 mb-8">Get tailored prep for any company. Culture insights, common questions, and what they value.</p>
        ...
        <div class="space-y-6">
          ...
          @if (companyOverview()) {
            <div class="rounded-xl border border-cyan-900/50 bg-cyan-950/20 p-5">
              <h3 class="font-semibold text-cyan-400 mb-2">Company Overview</h3>
              <p class="text-sm text-slate-300">{{ companyOverview() }}</p>
            </div>
          }
          ...
          @if (cultureInsights()) {
            <div class="rounded-xl border border-purple-900/50 bg-purple-950/20 p-5">
              <h3 class="font-semibold text-purple-400 mb-2">Culture Insights</h3>
              <p class="text-sm text-slate-300">{{ cultureInsights() }}</p>
            </div>
          }
          ...
          @if (whatTheyValue().length) {
            <div class="rounded-xl border border-emerald-900/50 bg-emerald-950/20 p-5">
              <h3 class="font-semibold text-emerald-400 mb-3">What They Value</h3>
              <ul class="space-y-2">
                @for (val of whatTheyValue(); track val) {
                  <li class="text-sm text-slate-300 flex items-start gap-2">
                    <span class="text-emerald-400 mt-0.5">&#9733;</span> {{ val }}
                  </li>
                }
              </ul>
            </div>
          ...
          @if (commonQuestions().length) {
            <div class="rounded-xl border border-slate-800 bg-slate-900/80 p-5">
              <h3 class="font-semibold text-white mb-3">Likely Interview Questions</h3>
              <ol class="space-y-3">
                @for (q of commonQuestions(); track q; let i = $index) {
                  <li class="text-sm text-slate-300 flex items-start gap-3">
                    <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-400">{{ i + 1 }}</span>
                    {{ q }}
                  </li>
                }
              </ol>
          ...
          @if (prepTips().length) {
            <div class="rounded-xl border border-slate-800 bg-slate-900/80 p-5">
              <h3 class="font-semibold text-white mb-3">Preparation Tips</h3>
              <ul class="space-y-2">
          ...
          @if (redFlags().length) {
            <div class="rounded-xl border border-red-900/50 bg-red-950/20 p-5">
              <h3 class="font-semibold text-red-400 mb-3">Watch Out For</h3>
              <ul class="space-y-2">
                @for (flag of redFlags(); track flag) {
                  <li class="text-sm text-slate-300 flex items-start gap-2">
                    <span class="text-red-400 mt-0.5">&#x26A0;</span> {{ flag }}
                  </li>
                }
              </ul>
            </div>
