...
class RiskControllerSessionIdDevTest {
    ...
    @Test
    void mockSessionIdAllowedInDev() throws Exception {
        mockMvc.perform(get("/api/risk/summary")
                        .param("sessionId", "mock-session-123"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.band").value("LOW"))
                .andExpect(jsonPath("$.missingSignals[0]").value("mock_mode"));
    }
}
